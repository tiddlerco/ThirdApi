 ...
 * 描述：字典错误枚举
 */
public enum SystemErrorCodeEnum implements ErrorCodeEnums {
    // @formatter:off
    ...
    private static final Map<Integer, SystemErrorCodeEnum> map = new HashMap<>();

    static {
        for (SystemErrorCodeEnum yn : SystemErrorCodeEnum.values()) {
            map.put(yn.code, yn);
        }
    }
    ...
    SystemErrorCodeEnum(Integer code, String desc) {
        this.code = code;
        this.desc = desc;
    }

    public static SystemErrorCodeEnum fromKey(String key) {
        if (SystemErrorCodeEnum.map.containsKey(key)) {
            return SystemErrorCodeEnum.map.get(key);
        }
        return null;
    }
    ...
    public ErrorCodeEnums setDesc(String desc) {
        this.desc = desc;
        return this;
    }
}
